...
_RUN_LIMITS: dict[str, dict[str, object]] = defaultdict(
    lambda: {
        "research_calls": 0,
        "revise_cycles": 0,
        "limit_reached": False,
...
def _build_research_followup_from_critique(
    ...
    lines = ["Revise the previous research using the critic's feedback."]
...
class RevisionLimitMiddleware(AgentMiddleware):
    ...
    def wrap_model_call(self, request, handler):
        ...
        counters = _RUN_LIMITS.setdefault(
            thread_id,
            {
                "research_calls": 0,
                "revise_cycles": 0,
        ...
        # Fix 3: inject research BEFORE calling the LLM to avoid wasted token spend
        ...
        messages = [response] if isinstance(response, AIMessage) else list(getattr(response, "result", []) or [])
        if not messages:
            return response
        ...
        # Fix 2: reset awaiting_save when rejection/edit detected so next model text isn't mis-saved
    ...
    def wrap_tool_call(self, request: ToolCallRequest, handler):
        ...
        if tool_name == "research":
            counters = _RUN_LIMITS.setdefault(
                thread_id,
                {"research_calls": 0, "revise_cycles": 0, "limit_reached": False, "last_findings": None},
            )
            if (
